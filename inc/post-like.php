 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_daren_process_simple_like', 'daren_process_simple_like' );
add_action( 'wp_ajax_daren_process_simple_like', 'daren_process_simple_like' );
function daren_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'daren' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'daren' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'daren' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'daren' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'daren' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'daren' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'daren_show_user_likes' );
add_action( 'edit_user_profile', 'daren_show_user_likes' );
function daren_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'daren' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'daren' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
