...
=========================================================*/
function daren_opt( $id = null, $default = '' ) {
...
function daren_site_icon(){
	if ( ! has_site_icon() ) {
		$html = '';
		$icon_path = DAREN_DIR_ASSETS_URI . 'img/favicon.png';
...
=========================================================*/
function daren_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_daren_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function daren_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'daren_excerpt_length' ) ) {
	function daren_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'daren_posted_comments' ) ) {
    function daren_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','daren');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','daren');
            } else {
                $comments = esc_html__( '1 Comment','daren' );
            }
            $comments = '<i class="ti-comment"></i>'. $comments;
        } else {
            $comments = esc_html__( 'Comments are closed', 'daren' );
        }
...
/*===================================================
	Post embedded media
===================================================*/
function daren_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function daren_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'daren' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'daren' ) . ' </span>%',
...
/*====================================================
	Theme logo
====================================================*/
function daren_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'site_logo_117x28' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt=""></a>';
	}else{
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'. DAREN_DIR_ASSETS_URI .'img/logo.png" alt=""></a>';
	}
...
/*================================================
    Page Title Bar
================================================*/
function daren_page_titlebar() {
	if ( ! is_page_template( 'template-builder.php' ) ) {
		?>
        <section class="hero-banner breadcrumb breadcrumb_bg">
            <div class="container">
				<div class="row">
					<div class="col-lg-12">
						<div class="breadcrumb_iner text-center">
							...
							<?php
							if ( is_category() ) {
								single_cat_title( __('Category: ', 'daren') );

							} elseif ( is_tag() ) {
								single_tag_title( __('Tag Archive for - ', 'daren') );
								...
								echo esc_html__( 'Search for: ', 'daren' ) . get_search_query();

							} elseif ( ! ( is_404() ) && ( is_single() ) || ( is_page() ) ) {
								echo  get_the_title();

							} elseif ( is_home() ) {
								echo esc_html__( 'Blog', 'daren' );

							} elseif ( is_404() ) {
								echo esc_html__( '404 error', 'daren' );
...
=================================================*/
function daren_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function daren_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function daren_featured_post_cat( $class_name = null ){
...
========================================================*/
function daren_sidebar_opt(){

    $sidebarOpt = daren_opt( 'daren_blog_layout' );
    $sidebar = '1';
...
============================================================*/
function daren_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'daren_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function daren_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex col-lg-12">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'daren' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'daren' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('daren_blog_single_post_navigation') ) {
	function daren_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'daren' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'daren' ); ?></p>
...
/*===================================================
 Daren Comment Template Callback
 ====================================================*/
function daren_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'daren' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'daren'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'daren' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'daren' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'daren_replace_reply_link_class');
function daren_replace_reply_link_class( $class ) {
...
===========================================================*/
function daren_banner_post( $cat, $meta = true, $order = 'desc' ){
    ...
    if( $bBlog->have_posts() ){
        while( $bBlog->have_posts() ){
			$bBlog->the_post();
			$banner_post_img = get_the_post_thumbnail_url(get_the_ID(), 'banner_post2_1159x811');
	?>
		<div class="banner_post_<?php echo $i?> banner_post_bg_<?php echo $i?>" style="background-image: url( <?php echo $banner_post_img?> )">
			<div class="banner_post_iner text-center">
				<?php echo daren_featured_post_cat(); ?>
...
===========================================================*/
function daren_feature_post( $cat ){
    ...
    if( $fBlog->have_posts() ){
        while( $fBlog->have_posts() ){
			$fBlog->the_post();
			$sing_cont_class = ($i == 2) ? 'd-block d-sm-none d-lg-block' : '';		
	?>

		<div class="single_feature_post post_1 <?php $sing_cont_class?>">
			<?php 
				if ( $i == 1 || $i == 3 ) {
					echo has_post_thumbnail() ? the_post_thumbnail( 'featured_post_380x310', ['alt' => get_the_title()] ) : '';
				}
			?>
			<div class="post_text_1 <?php echo ( $i == 2 || $i == 3 ) ? 'pl_pr_30' : 'pr_30';?>">
				<?php echo daren_featured_post_cat(); ?>
...
===========================================================*/
function daren_category_post( $cat, $meta, $order, $cat_lbl ){
...
===========================================================*/
function daren_latest_blog( $not_cat, $pNumber = 5, $meta, $order ){
	...
	?>
			...
			<div class="single_post media post_3">
				<div class="single_post_img">
					...
					?>
					<?php echo daren_featured_post_cat( 'category_btn' );?>
				</div>
...
/*=========================================================
    Share Button Code
===========================================================*/
function daren_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
